...
- `Permanent` means **part-time**, never full-time. Casual is the other
  supported basis.
...
| `HIGA-POLICY-APPROVED-IMMUTABLE` | Approved calculations remain stable after rates/config change; drafts recalculate. | Bepis audit policy | Permanent, casual, imported override | Persist exact paid-time segments. | Persist resolved quantities, rates, amounts, source and version. | [#234][i234] | `Test.TimesheetPayLedgerSpec` | Approved ledger only for final exports. | Approved ledger only for submission. |
...
| `HIGA-POLICY-OUTPUT-CONSERVATION` | Every positive approved component maps to exactly one output; fixed additions/missed-break money do not become Staff Hours. | Bepis output policy | Permanent, casual, imported override | SH equals approved paid-time minutes, including minimum top-ups. | DE/XR quantities and rounded lines reconcile to the approved ledger. | [#237][i237], [#241][i241] | `Test.Controller.FixedExportGoldenSpec`, Xero preview/readiness/submission specs | SH and DE exact goldens. | Exactly-once, idempotent mapping. |
...
| `HIGA-POLICY-GENERIC-TIME` | UI choices remain quarter-hour aligned, but the calculator is generic and no quarter-hour database invariant is added. | Bepis input policy | Permanent, casual, imported override | Preserve exact valid authoritative interval duration. | Quantities derive from exact duration. | [#229][i229], [#230][i230] | `Test.WageEngine.ContractSpec`, `Test.VenueTimeSpec` | Exact quantities rendered to output precision. | Exact quantities sent at supported precision. |
...
| `HIGA-EXCL-TOIL` | Time off instead of paid overtime. | [cl 28.5 and Schedule F][award] | Permanent, casual | No time bank/top-up. | No TOIL component. | [#240][i240] remains on hold | Future overtime contract | Do not emit. | Do not emit. |
| `HIGA-EXCL-OVERTIME` | Every daily, weekly, rostered-hours and period-level overtime rule. | [cl 10.13(b), 11.4 and 28][award] | Permanent, casual | No overtime allocation in the immediate engine. | No overtime component; deferred to hold issue [#240][i240]. Add no permanently failing overtime tests. | [#240][i240] | None until explicitly authorized | No overtime line. | No overtime item. |
