...
| `HIGA-POLICY-OUTPUT-CONSERVATION` | Every positive approved component maps to exactly one output; fixed additions/missed-break money do not become Staff Hours. | Bepis output policy | Permanent, casual, imported override | **#237 target:** SH derives from approved exact paid time, including minimum top-ups, then applies final-bucket quarter-hour export rounding. Current legacy SH retains exact aggregate quantities through its existing formatting. | **#237 target:** DE/XR quantities and amounts reconcile through the same explicit export-rounding transform. Current legacy DE/XR retain exact aggregates through existing output formatting/protocol serialization. | [#237][i237] | `Test.Controller.FixedExportGoldenSpec`, Xero preview/readiness/submission specs | SH and DE exact goldens. | Exactly-once, idempotent mapping. |
...
| `HIGA-POLICY-GENERIC-TIME` | UI choices remain quarter-hour aligned, but the calculator is generic and no quarter-hour database invariant is added. | Bepis input policy | Permanent, casual, imported override | Preserve exact valid authoritative interval duration and persist exact-second ledger facts. | Calculation quantities derive from exact duration. | [#229][i229], [#230][i230] | `Test.WageEngine.ContractSpec`, `Test.VenueTimeSpec` | **#237 target:** final output buckets defensively round once to the nearest 15 minutes; current legacy outputs retain exact aggregates. | **#237 target:** hourly output buckets use the same rounding and recompute line amount consistently; current legacy Xero uses exact aggregate protocol quantities. |
