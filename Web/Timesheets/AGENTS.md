...
- Keep chooser, ordinary, roster-prefill and review operations in `EntryWorkflow.hs` and completion/calendar HTTP in
  ...
  Controllers must not branch on nested materialization results.
- Preserve both new and existing snapshot completion kinds and convergent retry
  ...
  Roster-prefill Save always creates an unapproved entry; approval remains a separate persisted-entry action.
