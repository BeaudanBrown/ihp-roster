...
- For server-mutating UI that can leave another mounted copy stale, use the live-fragment path by default while keeping the actor path immediate. Add the controller surface in this order: feature-local fragment enum, typed surface definition, fragment GET action with typed auth, rendered surface metadata, actor-path HTMX response, post-commit typed broadcast/mutation helper, Hspec contract coverage for config/target/auth drift, then Playwright coverage when browser behavior such as resync, focus protection, or no-full-page navigation is part of the feature.
