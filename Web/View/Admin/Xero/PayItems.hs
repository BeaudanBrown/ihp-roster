...
import Application.Helper.FrontendContract.Overlay (ImportXeroPayItemsOverlay,
...
xeroPayItemOverlayActionRoute actionUrl =
    OverlayActionRoute
...
renderLoadXeroPayItemImportForm =
    renderOverlayActionForm
        (overlayActionByMarker @LoadXeroPayItemImportOverlay)
        (xeroPayItemOverlayActionRoute (pathTo OpenXeroPayItemImportAction))
            { overlayActionRouteFields = [OverlayFieldValue ("loadCandidates", "true")]
            }
        mempty
...
renderImportXeroPayItemsForm candidates =
    renderOverlayActionForm
        (overlayActionByMarker @ImportXeroPayItemsOverlay)
        (xeroPayItemOverlayActionRoute (pathTo ImportXeroPayItemsAction))
            { overlayActionRouteExtraAttrs = [("id", "xero-imported-pay-items-import-form")]
            }
        [hsx|
