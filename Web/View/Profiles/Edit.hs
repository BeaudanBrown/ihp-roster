        ...
        renderAppPage (AppPageConfig
            ...
            , appPageBody =
                renderAppPanel AppPanelConfig
                    ...
                    , appPanelClass = ""
                    , appPanelBodyClass = ""
                    , appPanelBody = renderProfileContentFragment staff currentUserEmail preferenceWeekdays preferenceSections selectedShiftPreferenceKeys passkeys leaveRequests leaveRequestForm openSection
                    }
            })

renderProfileContentFragment :: Staff -> Text -> [PreferenceWeekday] -> [StaffPreferenceGroupSection] -> [Text] -> [Passkey] -> [LeaveRequest] -> LeaveRequest -> Text -> Html
renderProfileContentFragment staff currentUserEmail preferenceWeekdays preferenceSections selectedShiftPreferenceKeys passkeys leaveRequests leaveRequestForm openSection = [hsx|
...
renderPasskeyManagement :: [Passkey] -> Html
renderPasskeyManagement passkeys = [hsx|
    <div class="app-form-width">
        <div class="js-passkey-register"
             ...
             data-success-redirect={appendQueryParams (pathTo EditProfileAction) [("section", "security")]}>
            <button type="button" class="btn btn-outline-primary js-passkey-register-button">Add passkey</button>
        </div>
        <div id="profile-passkey-status" class="alert d-none mt-3"></div>
        <div class="mt-4">
