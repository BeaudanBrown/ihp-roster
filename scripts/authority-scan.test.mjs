...
function fixture(t, { checkout = true } = {}) {
    ...
    if (checkout) {
        // Only scanner inputs: no application checkout, assets, build or Git state.
        for (const script of ['frontend/surface-guardrails', 'haskell/typed-contract-authority-check', 'lib/authority-scan.sh']) {
            const path = `Config/nix/scripts/${script}`;
            put(root, path, readFileSync(join(repo, path)));
        }
        for (const path of [
            'Application/Schema.sql', 'Config/nix/weeder-baseline.tsv',
            'Application/Helper/FrontendContract/AppShell.hs',
            'Application/Helper/FrontendContract/Surface/Admin.hs',
            'Web/Controller/Admin.hs', 'Web/Types.hs', 'Web/View/Admin/VenueSettings.hs',
            'Web/Controller/Admin/Xero/Timesheets.hs', 'Web/View/Admin/Xero/TimesheetPreparation.hs',
            'frontend/ts/generated/contracts.ts',
        ]) put(root, path, '');
        for (const path of [
            'Application/Support', 'Web/Admin', 'Web/Billing', 'Web/LeaveRequests',
            'Web/Profiles', 'Web/RosterWeeks', 'Web/Timesheets', 'Web/View/Feedback',
            'Web/View/Profiles', 'Web/View/Staff', 'Web/View/RosterWeeks', 'e2e', 'static/css',
            ...['Admin', 'Billing', 'LeaveRequests', 'Profile', 'Roster', 'SelfServiceLeave', 'Support', 'Timesheets']
                .map((name) => `Application/Helper/FrontendContract/Surface/${name}/Generated`),
        ]) mkdirSync(join(root, path), { recursive: true });
        for (const kind of ['Action', 'Intent']) {
            const base = 'Test/Support/FrontendSurfaceAdapterFixture';
            put(root, `${base}/Generated/${kind}.hs`, '-- @generated by Application.Helper.FrontendContract.Surface.HaskellAdapter.Generator\n');
            put(root, `${base}/${kind}.hs`, `import Test.Support.FrontendSurfaceAdapterFixture.Generated.${kind}\n`);
        }
        put(root, 'Application/Helper/FrontendContract/Surface/Request/Runtime.hs',
            'import Application.Helper.FrontendContract.Surface.Request.Runtime.Internal\n');
    }
...
test('surface token counts require two production uses and exclude generated/test copies', (t) => {
    const root = fixture(t);
    // Generated tokens require an import and use in non-generated source.
