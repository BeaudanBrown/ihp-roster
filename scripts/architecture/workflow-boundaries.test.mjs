...
test("both delivered pilots and the existing Billing phase adapter fit the import contract", () => {
  ...
  for (const name of ["ReadModel", "Responses", "Mutations"]) {
    assert.ok(result.modules.some(({ module, owner }) => module === `Web.Billing.${name}` && owner === "Billing"));
  }
  for (const name of ["Request", "Responses", "Mutations"]) {
    assert.ok(result.modules.some(({ module, owner }) => module === `Web.LeaveRequests.${name}` && owner === "Unavailability"));
  }
