    ...
    describe "WageSourcePolicy HIGA-SOURCE-FRESHNESS pure wage-source policy" do
        it "accepts an FWC complete success exactly 8 days old and blocks one second beyond the boundary" do
            let clock = PolicyClock (utc 2026 6 29)
                ...
                input = healthyInput { fwcSnapshots = [completeAt boundary], dataVicSnapshots = [dataVicCompleteAt 2026 clock.now] }
            evaluateWageSourcePolicy clock input `shouldBe` sourcesReady
            let staleInput = input { fwcSnapshots = [completeAt (addUTCTime (-1) boundary)] }
        ...
        it "requires the first full Monday pay week after 1 July to use a complete success on or after 1 July" do
            let clock = PolicyClock (utc 2026 7 6)
                beforeJuly = completeAt (utcSeconds 2026 6 30 (23 * 60 * 60 + 59 * 60 + 59))
                annualInput =
                    healthyInput
                ...
                diagnostic =
                    FwcAnnualRefreshMissing
                        ...
                        , latestCompleteFwcSuccess = Just beforeJuly.completedAt
                        }
            evaluateWageSourcePolicy clock annualInput `shouldBe` blockedBy [diagnostic]
            evaluateWageSourcePolicy clock (annualInput { fwcSnapshots = [completeAt (utc 2026 7 1)] })
        ...
        it "ignores failed and incomplete candidates instead of replacing the last complete success" do
            let clock = PolicyClock (utc 2026 5 1)
                oldSuccess = completeAt (addUTCTime (negate (fwcMaximumAge + 1)) clock.now)
                input =
                    healthyInput
                        { fwcSnapshots =
                            [ oldSuccess
                            , SourceSnapshot clock.now FailedCandidate
                            , SourceSnapshot clock.now IncompleteCandidate
                            ]
                        }
            evaluateWageSourcePolicy clock input
                `shouldBe` blockedBy [FwcSnapshotStale oldSuccess.completedAt fwcMaximumAge]
...
propFailedCandidatesIgnored (NonNegative rawSeconds) =
    let seconds = fromIntegral (rawSeconds `mod` (20 * 24 * 60 * 60))
        ...
        input = healthyInput { fwcSnapshots = [completeAt (addUTCTime (negate seconds) clock.now)] }
        failed = SourceSnapshot (addUTCTime 1 clock.now) FailedCandidate
...
healthyInput =
    WageSourcePolicyInput
        ...
        , fwcSnapshots = [completeAt (utc 2026 5 1)]
...
dataVicCompleteAt targetYear completedAt =
    DataVicSnapshot targetYear (completeAt completedAt)
