...
generatePayrollExportJob ::
    (?application :: WebApplication, ?mocking :: MockContext WebApplication, ?context :: RequestContext, ?applicationContext :: ApplicationContext, ?modelContext :: ModelContext) =>
    User ->
    Venue ->
    Text ->
    Int ->
...
generatePayrollExportJob user venue reportSlug weekOffset = do
    ...
    query @ExportJob |> orderByDesc #createdAt |> fetchOne

readExportFixtureText :: FilePath -> IO Text
readExportFixtureText relativePath =
    TextIO.readFile ("Test/Fixtures/exports/" <> relativePath)

decodeExportZipArchive :: ExportJob -> Zip.Archive
decodeExportZipArchive exportJob =
    exportJob.fileContents
        |> fromMaybe ""
        |> encodeUtf8
...
readZipEntryText :: FilePath -> ExportJob -> Text
readZipEntryText entryPath exportJob =
    decodeExportZipArchive exportJob
        |> Zip.findEntryByPath entryPath
