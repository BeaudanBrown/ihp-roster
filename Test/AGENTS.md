...
TEST_FEEDBACK_LANE=routine bash ./bin/in-env hspec-test  # expert metadata diagnostic only; never merge-gate evidence
...
Database selection (`all`/`pure`/`db`) and feedback selection (`all`/`routine`/`acceptance`) are orthogonal metadata dimensions. `TEST_FEEDBACK_LANE` remains a low-level expert diagnostic interface; do not add stable routine-only or acceptance-only wrappers. Measured acceptance-only and DB-only selections cost approximately as much as complete Hspec while still omitting required evidence, and routine-only omits every mandatory A/R/T/P/B category. `hspec-test` therefore remains the single user-facing mandatory Hspec command and required CI gate. The point-in-time measurements and decision are archived in `docs/archive/hspec-feedback-lane-decision-2026-07-30.md`.

A non-complete selection prints `mandatory-acceptance-excluded=[...]`, conservatively listing an automated invariant whenever any registered owner is outside the selection. Billing invariant `B6` is complete only because the reviewed Dahlia `subscription.created`, `subscription.updated`, and `subscription.deleted` fixtures all run through the signed webhook/database path. `mandatory-acceptance-composed=[T4]` records that leave approval/invalidation/rendering evidence is intentionally composed across `Conflict`, `LeaveRequestsController`, and roster fragments rather than one end-to-end example. `B8` remains operator/runbook evidence outside Hspec metadata. Hspec `--match`, `--skip`, focused-only, and rerun filters set `coverage-scope=focused-conservative` and conservatively report every automated invariant as omitted because suite-level metadata cannot prove which individual examples ran. Never describe a focused/routine/pure/DB-only result as the complete Hspec gate.
...
- Set `TEST_POSTGRES_MODE=external` together with an explicit
  ...
  Neither an inherited `PGHOST` nor `TEST_DB_SOCKET` alone opts out of managed
  mode; both the external mode and socket must be explicit. This prevents the
...
2. Add one `pureSuite` or `databaseSuite` entry to `allSuites` with a named `SuiteDefinition` record. Keep every record field explicit; the helpers set only pure/database isolation. A pure suite must pass with PostgreSQL unavailable; any suite that constructs an IHP database context remains database-backed until it is actually moved to a no-database context.
...
4. Classify the responsibility as `RoutineCorrectness` for focused helper/wiring feedback or `BroadAcceptance` for broad deterministic scenarios, cross-component acceptance, or suites that own irreplaceable `specs/09-testing-and-acceptance.md` evidence. This classification is diagnostic metadata, not permission to replace complete Hspec with a partial feedback lane.
5. Assign one broad `InvariantFamily` and list the exact `A*`/`R*`/`T*`/`P*`/`B*` invariants the suite owns. An empty list is valid for infrastructure or focused correctness, but registry validation rejects any mandatory automated invariant with no complete or explicitly partial declaration and rejects removal of the final suite representing any `InvariantFamily`. A suite that is the irreplaceable owner of an invariant belongs in `BroadAcceptance`, even when it is pure. Set `definitionPartialInvariants` only for a proven, explicitly catalogued temporary gap; partial evidence remains visible and never counts as complete ownership. Composed evidence such as `T4` must retain at least two registered owners and is reported separately.
