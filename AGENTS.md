...
- `xero` is venue-owner-only in the header; `admin` is role-gated (admin-only visibility); `timesheets` and `admin` may route to placeholder pages until fully implemented
...
- Xero connection management is restricted to the current venue owner or a super admin. The top-level Xero header button is visible only to the current venue owner; venue admins must not see Xero controls in Admin.
