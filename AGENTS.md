...
- **Exploratory browser work**: Use `bash ./bin/in-env pwcli ...` for ad hoc Playwright CLI sessions, targeted screenshots, and live selector discovery. `pwcli-auth-save <manager|worker|admin|support>` depends on the `seed-dev app` role accounts; manager/admin/worker use `password123`, while the seeded support super-admin uses `admin`. The isolated `e2e` suite still uses `test-password-123` against `app_e2e`. After saving state, open a pre-authenticated session with `bash ./bin/in-env pwcli-auth-open <role> <path>`.
