...
`FrontendSurface` invalidations are semantic and mount-resolved. Subscriptions,
websocket messages, and actor event details identify the logical scope and
canonical surface-local fragment keys, never concrete DOM target ids or GET URLs:
...
The generated websocket DTO and actor event detail share these semantic
boundaries:

- `scope` is the closed generated `SurfaceScope` envelope for the declared
  `FrontendSurface` scope.
...
All registered live surfaces now use this key-only protocol. URL, target,
selector, defer, and protection values remain local mount metadata. Stale browser
tabs from a previous deploy may stop functioning until reload; failures stay safe
because transport payloads cannot provide executable refetch authority.
