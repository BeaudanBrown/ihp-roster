...
- The `Application/Helper/View.hs` compatibility-wrapper cleanup is broken out
  into `docs/archive/plans/61-view-helper-split.md`.
...
- `Application/Helper/View.hs` still contains implementations even though the
  ...
  `docs/archive/plans/61-view-helper-split.md` / `ir-2usx`.
- `static/app.js` remains the frontend catch-all. Keep this in `ir-9f7z` and
  `docs/archive/plans/62-live-fragment-system-refactor.md`; do not start a bundler migration
...
- `Application/Schema.sql` is necessarily large but lacks a top-level navigation
  ...
  `docs/archive/plans/60-v1-schema-hardening.md` / `ir-caf4`.
