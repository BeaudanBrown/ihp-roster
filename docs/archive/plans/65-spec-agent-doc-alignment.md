...
4. `docs/archive/plans/` are durable design history. Prefer short "Current status" or
...
`docs/archive/plans/68-append-only-pay-config-versioning.md` and `ir-hw2v`.
...
- Add a current-status note to `docs/archive/plans/58-xero-connection-foundation.md`.
...
> `AGENTS.md`, then `docs/archive/plans/65-spec-agent-doc-alignment.md`, then `tk show
