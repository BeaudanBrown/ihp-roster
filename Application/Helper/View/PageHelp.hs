...
pageHelpTopics =
    [ topic "roster" "Roster"
        [ section HelpEveryone "Viewing"
            ...
            , iconItem HelpEveryone "bi-eye" "Own-shift highlight" "Find your live shifts" "Live rosters highlight your own assigned shifts by default. Turn this off in Settings if preferred. A manager hovering or pinning another staff member temporarily takes precedence; leaving or unpinning restores your own highlight. Draft rosters do not apply a default highlight."
            ]
        , section HelpManagerPlus "Planning"
            [ iconItem HelpManagerPlus "bi-layout-text-window-reverse" "Templates" "Design reusable Day or Week plans" "Use the Templates side-panel tab to create, edit, delete, or apply shared plans. Select a Day template then choose an outlined day, or select a Week template to confirm the viewed week immediately. Dragging a card reaches the same confirmation. Escape, Cancel, reselecting the card, or clicking invalid space cancels Day targeting. Templates cannot be applied while the roster is Live."
            ...
            , buttonItem HelpManagerPlus "bi-envelope" "Email roster" "Email the live roster" "Open Settings, then Share roster. Confirm the venue, roster group, week, recipient count, skipped count, and latest attempt before queueing delivery. Each person receives only their shifts plus every Open shift from that snapshot. Queued mail still sends if the roster returns to draft. Email roster is unavailable for draft rosters, when nobody is eligible, or while delivery is still in progress." "btn btn-outline-primary" (Just "bi-envelope") "Email roster"
            , buttonItem HelpManagerPlus "bi-fullscreen" "Side panel" "Expand the roster workspace" "On desktop, use the SidePanel button at the top right to hide the Staff/Templates/Settings panel temporarily. Use it again or press Escape while working in the expanded roster to restore the panel. On phones the panel stays stacked below the roster." "btn btn-outline-secondary" (Just "bi-fullscreen") ""
    ...
    , topic "timesheets" "Timesheets"
        [ section HelpEveryone "Timesheet tasks"
            ...
            , iconItem HelpEveryone "bi-sliders" "Settings panel" "Choose what appears" "Use the Settings side panel to toggle Hide approved or Show suggestions. These choices are saved to your account and remain active after navigation or reload."
        ...
        , section HelpManagerPlus "Review tasks"
            [ iconItem HelpManagerPlus "bi-people" "Staff side panel" "Review and locate staff entries" "The Staff panel always lists every active Timesheet-eligible staff member with total entries and approved entries in parentheses. Hover or focus a row to highlight matching cards, or use the eye to pin the highlight. Highlighting never filters the week; choose a staff member in Settings to narrow the current URL."
