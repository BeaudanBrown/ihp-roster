...
pageHelpTopics =
    ...
    , topic "timesheets" "Timesheets"
        ...
        , section HelpManagerPlus "Review tasks"
            [ iconItem HelpManagerPlus "bi-people" "Staff side panel" "Review and locate staff entries" "The Staff panel always lists every active Timesheet-eligible staff member with total entries and approved entries in parentheses. A roster-group filter narrows those counts; transient suggestions remain excluded. Hover or focus a row to highlight matching cards, or use the eye to pin the highlight. Highlighting never filters the week; use Settings to narrow the current URL by staff or active roster group. Turn Manager mode off instead of using a personal filter when you want worker-equivalent self-service. Ad-hoc entries appear only under All roster groups."
