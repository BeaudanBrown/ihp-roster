...
pageHelpTopics =
    [ topic "roster" "Roster"
        [ section HelpEveryone "Viewing"
            ...
            , iconItem HelpEveryone "bi-sliders" "Side panel tabs" "Choose a section" "On smaller screens, tap Roster tools at the bottom to open the shelf beneath navigation; tap its top bar again to close it. Select a section's icon to open it. The selected tab expands to show its name; the other tabs keep their icons."
            ...
            , iconItem HelpManagerPlus "bi-person-workspace" "Manager mode" "Switch between planning and self-service" "Use the switch at the top of Settings. Off keeps Published rosters and the Staff list visible, hides Draft detail and planning controls, and blocks roster management actions while keeping your own Timesheet and unavailability tools. The same saved mode applies on Timesheets."
            ]
        , section HelpManagerPlus "Planning"
            ...
            , buttonItem HelpManagerPlus "bi-broadcast" "Published switch" "Publish or return to Draft" "Publish when the roster is ready for staff and all assigned pay configuration warnings are resolved. Return it to Draft to remove its shifts from Timesheet prefill choosers." "btn btn-outline-success" Nothing "Published"
            ...
            , buttonItem HelpManagerPlus "bi-fullscreen" "Side panel" "Expand the roster workspace" "On desktop, use the SidePanel button at the top right to hide the Staff/Settings panel temporarily. Use it again or press Escape while working in the expanded roster to restore the panel. On smaller screens, tap Roster tools to open or close the full-height tools shelf. Pinning a staff eye highlight closes the shelf to reveal the roster." "btn btn-outline-secondary" (Just "bi-fullscreen") ""
            ...
            , buttonItem HelpManagerPlus "bi-sliders" "Settings tab" "Show warnings or wage estimates" "Open Settings in the staff panel, then use the Display controls. Personal roles can enable `Show my expected pay`; Admin and Owner in Manager mode can enable venue `Show expected wage estimates`. Estimates stay within the selected roster group and use the same calculation as draft Timesheets. Roster-only shifts are omitted; calculation errors for Timesheet-producing shifts appear beside the week total. Source-maintenance warnings are visible only to platform super admins. Staff eye controls highlight shifts only and never change pay totals." "btn btn-outline-secondary" (Just "bi-sliders") ""
    ...
    , topic "timesheets" "Timesheets"
        [ section HelpEveryone "Timesheet tasks"
            ...
            , buttonItem HelpEveryone "bi-calendar-check" "Roster-prefill chooser" "Use a Published roster shift" "Click a day's + bar to choose an eligible Published roster shift or a blank Timesheet. Roster cards use the standard Timesheet shape and open a form for review before saving." "btn btn-sm btn-outline-success" Nothing "Use this shift"
            , iconItem HelpEveryone "bi-people" "Authorized choices" "See the complete safe set" "Your shifts appear first. In Manager mode, authorized shifts for other Staff appear separately. Staff and roster-group filters never hide choices in this chooser."
            , buttonItem HelpEveryone "bi-plus-lg" "Day add control" "Choose a source before editing" "Click the + bar to open the chooser. A sole blank choice opens directly; even one roster shift keeps the chooser visible so source linkage stays explicit. If no eligible Staff can create Timesheets, a toast explains why creation is unavailable." "timesheet-day-add-bar" Nothing "+ Mon 1 Jan"
            , buttonItem HelpEveryone "bi-fullscreen" "Side panel" "Expand the Timesheets workspace" "On desktop, use the SidePanel button at the top right to hide the panel temporarily. Use it again or press Escape while working in the expanded main card to restore the panel. On smaller screens, tap Timesheet tools at the bottom to open the tools shelf; tap its top bar or press Escape to close it." "btn btn-outline-secondary" (Just "bi-fullscreen") ""
            , iconItem HelpEveryone "bi-sliders" "Settings panel" "Choose what appears" "Use the Settings side panel to choose Show approved and your estimated-pay display mode. Pay starts Hidden; choose visible Timesheets, all Timesheets in the week, or both. These choices are saved to your account and remain active after navigation or reload."
            , iconItem HelpManagerPlus "bi-person-workspace" "Manager mode" "Work on only your own time" "Use the switch at the top of Settings. Off clears staff and roster-group filters, shows only your own entries and eligible roster-prefill choices, and removes reassignment, manager notes, and approval actions. The same saved mode applies on Roster; turn it on again to start from All staff and All roster groups."
            , iconItem HelpEveryone "bi-cash-coin" "Estimated gross wage" "Review visible shift estimates" "The week header and each day can show persisted visible Timesheets, all authorized Timesheets in that period, or both. Roster-prefill choices never contribute. Visible totals follow Staff, roster-group, and Show approved filters; all totals ignore those presentation filters. Unavailable calculations are excluded and reported beside the partial amount."
        ...
        , section HelpManagerPlus "Review tasks"
            [ iconItem HelpManagerPlus "bi-people" "Staff side panel" "Review and locate staff entries" "The Staff panel always lists every active Timesheet-eligible staff member with total entries and approved entries in parentheses. A roster-group filter narrows those counts; transient roster-prefill candidates remain excluded. Hover or focus a row to highlight matching cards, or use the eye to pin the highlight. Highlighting never filters the week; use Settings to narrow the current URL by staff or active roster group. Turn Manager mode off instead of using a personal filter when you want worker-equivalent self-service. Ad-hoc entries appear only under All roster groups."
            , buttonItem HelpManagerPlus "bi-check-lg" "Approve" "Approve staff entries" "Use the green Approve button on an existing unapproved entry. Approval stops without creating or changing the entry when its pay calculation or authoritative wage sources are not ready." "btn btn-sm btn-outline-success timesheet-approval-toggle" Nothing "Approve"
    ...
    , topic "leave" "Unavailability"
        [ section HelpEveryone "Request tasks"
            ...
            , iconItem HelpEveryone "bi-table" "Request list" "Review unavailable periods" "Use this page to check each unavailable period's dates, staff member, status, and notes. On smaller screens, tap Unavailability tools to open the Staff and Settings shelf; tap its top bar again to close it."
    ...
    , topic "xero" "Xero"
        [ section HelpOwnerPlus "Xero tasks"
            ...
            , iconItem HelpOwnerPlus "bi-list-check" "Choose shifts" "Review exactly what is uploaded" "After choosing a period and any required pay-item account, the shift checklist opens automatically. Select the approved work and choose Confirm and submit; there is no additional confirmation screen. If preparation repairs an approval, review the updated checklist and confirm again. A new period resets the selection. Pay-item setup, preview and upload use only these shifts; changed approvals require fresh review. Confirming replaces selected employees' editable drafts in full, removing any unselected draft contents for those employees. Employees with no selected shifts remain untouched. A current draft pay run is required, but does not grant permission to edit a processed provider timesheet: processed timesheets cannot be replaced, even with a separate draft pay run. Use a custom workbook with the Xero pay-items sheet for manual entry instead; Bepis does not calculate payroll adjustments, tax, super or net pay."
            ...
            , iconItem HelpOwnerPlus "bi-send-check" "Prepare" "Submit draft timesheets" "Upload timesheets opens the guided workflow for staff decisions, pay-period selection, any required new-pay-item account choice, and draft submission. Choose from current draft pay runs in Xero; the newest period is selected by default and duplicate calendar/periods appear once. Continue is unavailable without a selectable draft period. Create a draft pay run in Xero first if the list is empty, or reconnect if prompted, then reopen Upload timesheets. Period dates come from the run, even when no approved shifts exist yet. Bepis rechecks that a matching draft run remains available before submission. Bepis automatically attempts approval repair for selected shifts as preparation advances, using current pay facts and Xero mappings while preserving prior ledger history and the saved selection. Stale approvals and active provider writes are protected; incomplete repairs roll back. Remaining wage-source, calculation, approved-ledger, and mapping problems still block unsafe submission, without per-Timesheet repair buttons. Bepis selects approved entries by Operational day and keeps every overnight entry whole in that provider-period position while actual worked times determine each pay item and quantity. Review submitted drafts in Xero. Export and Xero history never locks an entry: correct it, reapprove it, then start a fresh preparation to update the matching Xero draft. Bepis does not remove an obsolete Xero draft when no approved local entries remain. If an entry was approved before its Xero pay item was available, preparation safely binds its sealed wage facts once the mapping is ready; no reapproval is needed. An approved entry already pinned or bound to a Xero rate that is no longer available must be corrected and reapproved; Bepis never silently remaps it."
            ]
