...
pageHelpTopics =
    ...
    , topic "xero" "Xero"
        [ section HelpOwnerPlus "Xero tasks"
            ...
            , iconItem HelpOwnerPlus "bi-arrow-repeat" "Sync" "Sync payroll reference data" "Bepis starts a background refresh after connection and schedules another around six days after each successful snapshot. You can also start one from the connection shell; repeated requests join the same refresh. Items removed or made inactive in Xero remain in payroll history but must be replaced before new use."
