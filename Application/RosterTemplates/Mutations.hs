...
lockRosterTemplateContentReferenceRows rosterGroupId shiftTypeIds staffIds = do
    ...
    _awardLevelLocks :: [Only UUID] <- unsafeSqlQuery
    ...
    _importedPayItemLocks :: [Only UUID] <- unsafeSqlQuery
...
lockRosterTemplateApplicationRows templateId venueId rosterGroupId windowStart windowEnd mappedShiftTypeIds = do
    ...
    _timesheetLocks :: [Only UUID] <- unsafeSqlQuery
    ...
    _staffLocks :: [Only UUID] <- unsafeSqlQuery
        ...
        (Only (unpackId templateId))
    _leaveLocks :: [Only UUID] <- unsafeSqlQuery
...
lockTargetRows venueId rosterGroupId windowStart windowEnd = do
    _dayLocks :: [Only UUID] <- unsafeSqlQuery
        ...
        (unpackId venueId, unpackId rosterGroupId, windowStart, windowEnd)
    _laneLocks :: [Only UUID] <- unsafeSqlQuery
        ...
        (unpackId venueId, unpackId rosterGroupId, windowStart, windowEnd)
    _slotLocks :: [Only UUID] <- unsafeSqlQuery
...
lockRosterTemplateName rosterGroupId normalizedName = do
    ...
    lockResults :: [Only Bool] <- unsafeSqlQuery
