...
orient jsonOutput = do
    ...
    classified <- forM openChildren $ \child -> do
        ...
        let openBlockers = filter ((== "open") . state) blockers
            reasons = (["assigned" | not (null (assignees child))]
                <> ["committed-work-awaiting-resolution" | number child `elem` committed]) :: [Text.Text]
            summary = IssueSummary (number child) (title child) (html_url child) (map login (assignees child))
        pure (summary, openBlockers, reasons)
    let ready = [summaryJson issue | (issue, blockers, reasons) <- classified, null blockers, null reasons]
        ...
        result = object
            ...
            , "selection" .= object ["allowed" .= not interrupted, "requiresUserChoice" .= True]
...
printHuman parent record ahead interrupted ready active blocked = do
    ...
    if interrupted
        ...
        else putStrLn "READY — choose explicitly"
    mapM_ (putIssue "  ") ready
    putStrLn "ACTIVE / INTERRUPTED"
    mapM_ (putIssue "  ") active
    putStrLn "BLOCKED"
    mapM_ (putIssue "  ") blocked
    putStrLn "Wait for the user; do not begin, close, or move issues automatically."
    putStrLn "After implementation: commit, present verification evidence, and request approval before closure."
    putStrLn "After approved closure: refresh orientation; proceed with an explicitly selected next issue, otherwise request selection."
  where
...
data IssueSummaryWire = IssueSummaryWire { wireNumber :: Int, wireTitle :: Text.Text }
instance FromJSON IssueSummaryWire where
    parseJSON = withObject "IssueSummary" $ \value -> IssueSummaryWire <$> value .: "number" <*> value .: "title"
