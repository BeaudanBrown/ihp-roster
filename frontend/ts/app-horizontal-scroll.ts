// @ts-nocheck
export function parsePositiveIntegerForHorizontalScroll(value) {
    const parsed = Number.parseInt(value || '', 10);
    return Number.isFinite(parsed) && parsed > 0 ? parsed : null;
}

export function clampHorizontalScrollLeft(scrollLeft, scrollWidth, clientWidth) {
...
(function enableHorizontalScroll() {
    ...
    const pointerSnapContainers = new Map();
    const touchSnapContainers = new Map();
    let activeDrag = null;
