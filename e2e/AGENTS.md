...
- `e2e-fast` sets `E2E_TIER=fast`, selects desktop plus canonical Pixel, defaults retries to zero, and stops each shard after its first failure for immediate agent feedback; set `PLAYWRIGHT_RETRIES=1` or use full `e2e` when collecting flake/broad-failure evidence. It does not change focused-argument shard detection; `e2e` uses `E2E_TIER=full` with normal retry and complete-failure evidence by default
...
- Canonical E2E runs, including focused one-shard checks, default to a compiled app executable under the native run directory reported by `e2e-runtime`, instead of live-reload `RunDevServer`. Compatible application and Stripe-mock objects/interfaces are reused from the fingerprinted `build/Verification` cache shared with typecheck and normal Hspec; `.devenv/e2e/<run-id>/` receives only the merged durable report. This avoids slow GHCi startup plus file-watcher/schema-codegen races against the shared working tree. Set `E2E_SERVER_MODE=dev` only when intentionally debugging the dev-server path.
...
- For FrontendSurface surfaces, cover the browser contract when behavior changes: the rendered owner has `data-bepis-surface-config`, no alternate mount protocol is emitted, subscriptions are sent from the generated config, matching HTMX requests receive `X-Live-Update-Client-Id`, resync refreshes the declared fragments, and focused-field protection defers only the protected fragment. For readiness, observe the `app:live-update-debug` `subscription_added` event before navigation; do not expect a client-id DOM attribute.
