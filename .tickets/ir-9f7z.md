...
- The app-runtime hotspot is now primarily `static/app.js`; keep splitting by
  concern while preserving the existing script-loading model.
- Coordinate with `ir-2vyr` for reusable OOB/fragment helpers and `ir-1i03` for
  admin mutation response patterns.
- Do not introduce a bundler as part of this maintenance pass unless a separate
  product/ops decision changes the settled direction.
...
**2026-04-30T02:07:33Z**

Runtime refactor children are complete: live-update hardening/protocol cleanup, script split, feature-local roster/timesheet extraction, and lifecycle/live-fragment verification. Final verification included node --check, typecheck, focused Hspec, and targeted Playwright live-fragment specs.
